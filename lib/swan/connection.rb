# This class represents a Swan API Connection.
class Swan::Connection
  ...
  headers(
      ...
      'User-Agent' => "ub0r/Swan-API #{Swan::VERSION}"
  )
  ...
  def to_s
    "Swan::Connection { :url => #{url} :options => #{options} }"
  end
  ...
  def parse_response(response)
    if response.success?
      ...
      raise Swan::Error.from_response(response)
    end
  end
